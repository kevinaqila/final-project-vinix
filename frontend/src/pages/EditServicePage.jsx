...
const EditServicePage = () => {
  ...
  const { id } = useParams();
  const { selectedService, isLoading, fetchServiceById, updateService, isLoading: isSubmitting } = useServiceStore();
  ...
  useEffect(() => {
    if (selectedService && !isInitializedRef.current) {
      ...
      setFormData({
        ...
        clientUploads: selectedService.clientUploads || [],
        freelancerDelivers: selectedService.freelancerDelivers || [],
        packages: {
          basic: {
            name: "basic",
            price: selectedService.packages?.basic?.price || "",
            description: selectedService.packages?.basic?.description || "",
            deliveryTime: selectedService.packages?.basic?.deliveryTime || "",
            features: selectedService.packages?.basic?.features || [""],
            revisions: selectedService.packages?.basic?.revisions || 0,
          },
          standard: {
            name: "standard",
            price: selectedService.packages?.standard?.price || "",
            description: selectedService.packages?.standard?.description || "",
            deliveryTime: selectedService.packages?.standard?.deliveryTime || "",
            features: selectedService.packages?.standard?.features || [""],
            revisions: selectedService.packages?.standard?.revisions || 1,
          },
          premium: {
            name: "premium",
            price: selectedService.packages?.premium?.price || "",
            description: selectedService.packages?.premium?.description || "",
            deliveryTime: selectedService.packages?.premium?.deliveryTime || "",
            features: selectedService.packages?.premium?.features || [""],
            revisions: selectedService.packages?.premium?.revisions || 3,
          },
        },
      });
    }
  ...
  return (
    <DashboardLayout>
      <div className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Edit Jasa</h1>
          <p className="text-gray-600 mt-2">Perbarui informasi jasa Anda</p>
        </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-600">Perbarui paket harga untuk memberikan fleksibilitas kepada klien</p>
                </div>
        ...
        <div className="flex justify-between mt-8">
          <button
            type="button"
            onClick={currentStep === 1 ? () => navigate("/freelancer/services") : prevStep}
          ...
          <div className="flex space-x-3">
            {currentStep < totalSteps ? (
              ...
              >
                {isSubmitting ? "Menyimpan..." : "Perbarui Jasa"}
              </button>
            )}
          </div>
        </div>
